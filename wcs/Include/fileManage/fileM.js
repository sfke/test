	...
	*/
	...
	function fileManage(){
		...
		this.getFileList = function (path,type) {
			...
			$.post(
			publicUrlInc+"/fileManage/ajax.php",
			{action:'get_data',
			 root:path,
			 type:type},
			function(data){
		...
		selected_file_type =null;
			...
			var str='<div><table width="100%"><tbody><tr><td width="2%"></td><td width="58%"><b>名称</b></td><td width="20%"><b>文件大小</b></td><td width="20%"><b>修改时间</b></td></tr></tbody></table></div>';
			for(i in file_arr){
				if(path==this.rootpath&&file_arr[i].name=='..') continue;
				if(file_arr[i].type=='dir')
					var ico = '<img src="'+publicUrlInc+'/fileManage/dir.gif"/>';
				else var ico = '<img src="'+publicUrlInc+'/fileManage/file.gif"/>';
				if(window.file_flag !==undefined&&window.file_flag==file_arr[i].name)
				{var flag_class = 'file_hover';window.file_flag=null; }
				else var flag_class ='';
				str+='<div id=n_'+i+' class="file_list '+flag_class+'"><table width="100%"><tbody><tr><td width="2%">'+ico+'</td><td width="58%">'+file_arr[i].name+'</td><td width="20%">'+file_arr[i].size+'</td><td width="20%">'+file_arr[i].lastmod+'</td></tr></tbody></table></div>';
 ...
 function check_name(x){
	...
	click: function() {
		...
		if(type=="文件"){
          var stype=selected_file_name.substr(selected_file_name.length-4);
		  var stype2=selected_file_name.substr(selected_file_name.length-3);
				if(stype=="html"||stype==".css"||stype2==".js"){
				  $('.file_control #file_edit').css('display','inline-block');
		        }else{
		          $('.file_control #file_edit').css('display','none');
		        }
		}else{
		   $('.file_control #file_edit').css('display','none');
		}
	...
	$("#select_ok,#file_del,#file_rename,#file_new,#select_cancel,#file_edit,#file_edit_save").click(function(){
		...
		if(this.id == 'file_rename' ){ 
			if(window.selected_file_name==undefined) {alert("没有选中的文件");return ;}
			var x;
			x=prompt("输入新的文件夹/文件名字  （文件需加后缀 如 *.html）");
		...
		if(this.id == 'file_edit' ){
			...
			{root:path,
			 ...
			 name:selected_file_name},
			function(data){
				 $('.fileM_box').css('display','none');
                 $('.fileM_box2').css('display','block');
				 $("#html_info").html(data); 
