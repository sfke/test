<?php 
  session_start();
  if($_SESSION['mysqlpwd']==""){
   $_SESSION['mysqlpwd']= $_GET['cmysql'];
  }
  if($_SESSION['mysqlpwd']==""){
     $adminurl=str_replace('wcs/Include/dbtool','admin',"http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
...
//strip quotes if they set
 ...
 if ($_REQUEST['login']){
    if (md5($_REQUEST['pwd'])!=$ACCESS_PWD){
        $err_msg="密码不正确，请重试！";	 
    }else if(substr(md5($_REQUEST['pwd']), 0 , 3)!="655"){
		$err_msg="密码不正确，请重试！";
	}else{
       $_SESSION['is_logged']=true;
       loadcfg();
 ...
 */
 ...
 $sqldr.="<table class='res $w' style='width:99%;border-collapse:collapse;'>";
 $headers="<tr class='h'>";
...
.inv a{color:#FFF}
table.res{width:99%;border-collapse:collapse;}
table.wa{width:auto}
...
.dot{width:98%;border-bottom:1px dotted #000}
.ajax{text-decoration: none;border-bottom: 1px dashed;}
.qnav{width:30px}
.sqldr{width:98%;margin-left:20px;}
</style>
...
<link href="../../public/css/right.css" rel="stylesheet" />
</head>
...
<div class="right">
  <!--当前位置 begin-->
  <div class="weizhi" style="position:fixed; top:0; left:0; width:100%; _position:absolute; _top:expression(eval(document.documentElement.scrollTop+ (parseInt(this.currentStyle.marginTop,10)||0)));overflow:visible; ">
    <div class="weizhi_shadow">
      <div class="wz_left"></div>
      <div class="wz_middle"> <a href="#">网站首页</a>
        <div class="jt"></div>
        <a href="#" class="current">欢迎界面</a> </div>
      <div class="wz_right"></div>
      <div class="clear"></div>
    </div>
  </div>
  <!--当前位置 end-->
...
<div class="inv">
<a href="http://www.jltech.cn/" target="_blank"><b></b></a>
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | <a href="?<?php echo $xurl?>&q=show+databases">数据库</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option><?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183; <a href='$self?$xurl&db=$dbn"; ?>
<?php echo $z.'&q='.urlencode($SHOW_T)?>'>显示所有表</a>
<?php echo $z?>&shex=1'>导出</a>
<?php echo $z?>&shim=1'>导入</a>
<?php } ?>
 | <a href="?showcfg=1">设置</a>
<?php } ?>
<?php if ($GLOBALS['ACCESS_PWD']){?> | <a href="?<?php echo $xurl?>&logoff=1" onclick="logoff()">退出</a> <?php }?>
</div>
...
SQL语句 : &nbsp;<button type="button" class="qnav" onclick="q_prev()">&lt;</button><button type="button" class="qnav" onclick="q_next()">&gt;</button><br>
...
花时: <b><?php echo $reccount?></b> in <b><?php echo $time_all?></b> 秒<br>
...
特殊管理密码: <input type="password" name="pwd" value="">
...
<h3>设置</h3>
<div class="frm">
<label class="l">数据库用户名:</label><input type="text" name="v[user]" value="<?php echo $DB['user']?>"><br>
<label class="l">数据库密码:</label><input type="password" name="v[pwd]" value=""><br>
<div style="text-align:right"><a href="#" class="ajax" onclick="cfg_toggle()">高级设置</a></div>
<div id="cfg-adv" style="display:none;">
<label class="l">数据库:</label><input type="text" name="v[db]" value="<?php echo $DB['db']?>"><br>
<label class="l">MySQL主机地址:</label><input type="text" name="v[host]" value="<?php echo $DB['host']?>"> 端口: <input type="text" name="v[port]" value="<?php echo $DB['port']?>" size="4"><br>
<label class="l">字符集:</label><select name="v[chset]"><option value="">- 默认 -</option><?php echo chset_select($DB['chset'])?></select><br>
<br><input type="checkbox" name="rmb" value="1" checked> cookie 记录会保留 30 天
</div>
<center>
<input type="hidden" name="savecfg" value="1">
<input type="submit" value=" 确定 "><input type="button" value=" 取消 " onclick="window.location='<?php echo $self?>'">
</center>
</div>
</center>
<?php
...
<center>
<!--h3>导出 <?php echo $l?></h3-->
<textarea style="width:90%;height:100px;border:none;background:#F8F8F8;margin:10px 0px;">导出 <?php echo $l?></textarea>
<div class="frm">
...
<div>
<?php if ($t && !strpos($t,',')){?>
 <label><input type="radio" name="et" value="csv"> .csv </label>
<?php }else{?>
<label>&nbsp;( ) .csv</label> <small></small>
<?php }?>
</div>
<br>
<div><label><input type="checkbox" name="gz" value="1"> 打包成 .gz 格式</label></div>
<br>
<input type="hidden" name="doex" value="1">
<input type="hidden" name="t" value="<?php echo $t?>">
<input type="submit" value=" 下载 "><input type="button" value=" 取消 " onclick="window.location='<?php echo $self.'?'.$xurl.'&db='.$DB['db']?>'">
</div>
</center>
<?php
 print_footer();
...
<input type="submit" value=" 上传并导入 " onclick="return ays()"><input type="button" value=" 取消 " onclick="window.location='<?php echo $self.'?'.$xurl.'&db='.$DB['db']?>'">
</div>
<br><br><br>
<!--
