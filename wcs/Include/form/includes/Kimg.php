 ...
 *  @package    Controls
 */
class Zebra_Form_Kimg extends Zebra_Form_Control
{
     ...
     *  @return void
     */
    function Zebra_Form_Kimg($id, $default = '', $attributes = '')
    {
    ...
    {   $name = $this->attributes['name'];
        $imgUrl = !empty($this->attributes['value'])?$this->attributes[value]:C('SYS_DEFAULT_IMG');
        $source = '
        <script>
                    ...
                    editor.loadPlugin("image", function() {
                        editor.plugin.imageDialog({
                            ...
                            clickFn : function(url, title, width, height, border, align) {
                                K("#'.$name.'").val(url);
                                K("#'.$name.'_v").html("<img style=\'margin:10px;max-width:300px;border:1px solid #DDDDDD;padding:5px;background:#FCFCFC;\' src="+url+">");
                                editor.hideDialog();
                            }
                        });
                    });
        ...
        return $source.'<input ' . $this->_render_attributes() . ($this->form_properties['doctype'] == 'xhtml' ? '/' : '') . '><input type="button" class="button kimg_'.$name.'" value="浏览"/> <div id="'.$name.'_v" style="clear:both;" ><img style=\'margin:10px;max-width:300px;border:1px solid #DDDDDD;padding:5px;background:#FCFCFC;\' src='.$imgUrl.'></div>';
    }
}
?>
