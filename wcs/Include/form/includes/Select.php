...
class Zebra_Form_Select extends Zebra_Form_Control
{
     ...
     *  @return void
     */
    function add_options($options, $overwrite = false,$arr)
    {
        $this->map = $arr;
    ...
    function _generate(&$options, &$selected, $level = 0)
    {
        ...
        foreach ($options as $value => $caption) {
            ...
            // if entry is a standard option
                ...
                if($this->map!=null&&$this->map[$value]==0)
                $flag = 'disabled style="background:#E9F6F9;"';
                else 
                $flag = '';
                // create the appropriate code
                $content .= '<option '.$flag.' value="' . $value . '"' .
					...
					($selected != '' &&
    					...
    					) ? ' selected="selected"  style="background:#DBDBDB;" ' : ''
