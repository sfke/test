...
class Zebra_Form_Submit extends Zebra_Form_Control
{
    ...
    function toHTML()
    {
        $returnUrl = empty($this->attributes['returnUrl'])?"javascript:history.go(-1);":"location.href='".$this->attributes['returnUrl']."'";
        return '<input style="float:left;" ' . $this->_render_attributes() . ($this->form_properties['doctype'] == 'xhtml' ? '/' : '') . '><input  type="button" name="my_reset" id="my_reset" value="返回" class="reset" style="float:left;margin-left:100px;" onclick='.$returnUrl.' ><div style="clear:both;"></div>';
